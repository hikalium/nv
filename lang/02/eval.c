...
NV_ID NV_Lang02_OpFunc_infixOp(NV_ID *const p, NV_ID *const lastEvalVal,
                               const NV_ID *scope) {
...
NV_ID NV_Lang02_OpFunc_prefixOp(NV_ID *const p, NV_ID *const lastEvalVal,
                                const NV_ID *scope) {
  NV_ID op = NV_Dict_getByStringKey(p, "op");
  NV_ID opR = NV_Dict_getByStringKey(p, "opR");
  NV_ID result = NV_Dict_getByStringKey(p, "result");
  ...
  if (strcmp(opStr, "print") == 0) {
  ...
  } else if (strcmp(opStr, "printel") == 0) {
    isAnsNotInteger = 1;
    opR = NV_Term_getPrimNodeID(&opR, scope);
    NV_Term_print(&opR);
    putchar('\t');
    *lastEvalVal = opR;
    NV_globalExecFlag |= NV_EXEC_FLAG_SUPRESS_AUTOPRINT;
  } else if (strcmp(opStr, "println") == 0) {
    isAnsNotInteger = 1;
    opR = NV_Term_getPrimNodeID(&opR, scope);
    NV_Term_print(&opR);
    putchar('\n');
    ...
    *lastEvalVal =
        NV_Node_createWithStringFormat("prefix: No op for %s", opStr);
    return *lastEvalVal;
  }
  if (!isAnsNotInteger) {
    NV_ID ansNode = NV_Node_createWithInt32(ans);
...
NV_ID NV_Lang02_OpFunc_postfixOp(NV_ID *const p, NV_ID *const lastEvalVal,
                                 const NV_ID *scope) {
  NV_ID op = NV_Dict_getByStringKey(p, "op");
  NV_ID opL = NV_Dict_getByStringKey(p, "opL");
  NV_ID result = NV_Dict_getByStringKey(p, "result");
  ...
  if (strcmp(opStr, "++") == 0) {
    ...
    *lastEvalVal =
        NV_Node_createWithStringFormat("postfix: No op for %s", opStr);
    return *lastEvalVal;
  }
  if (!isAnsNotInteger) {
    NV_ID ansNode = NV_Node_createWithInt32(ans);
...
NV_ID NV_Lang02_OpFunc_cond(NV_ID *const p, NV_ID *const lastEvalVal,
                            const NV_ID *scope) {
...
NV_ID NV_Lang02_OpFunc_do(NV_ID *const p, NV_ID *const lastEvalVal,
                          const NV_ID *scope) {
...
NV_ID NV_Lang02_OpFunc_parentheses(NV_ID *const p, NV_ID *const lastEvalVal,
                                   const NV_ID *scope) {
  ...
  if (!NV_Term_isNotFound(&opL)) {
    ...
    NV_ID parsedBlock =
        NV_Lang_parseToCodeGraph(&codeBlock, &opDict, NV_Lang02_funcList);
    //
    NV_ID ansNode;
    {
      NV_ID subScope = NV_Variable_createSubScopeOf(scope);
...
NV_ID NV_Lang02_OpFunc_doNothing(NV_ID *const p, NV_ID *const lastEvalVal,
                                 const NV_ID *scope) {
