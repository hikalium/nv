...
NV_ID NV_tokenize(const NV_ID *cTypeList, const char *input) {
  ...
  for (i = 0;; i++) {
    ...
    if (cType != lastCType || cType == 2 || lastCType == 2 || cType == 0 ||
        lastCType == 0) {
      if (input + i - p != 0) {
...
int NV_checkAndPrintErrorOfCodeBlock(const NV_ID *code)
{
        ...
        for(i = 0; ; i++){
                ...
                if(!NV_NodeID_isEqual(&failedOp, &NODEID_NOT_FOUND)){
                        failedReason = NV_Dict_getByStringKey(&t,
"failedReason"); NV_Term_print(&failedOp); printf(" : ");
...
void NV_evalLoop(const NV_ID *opList, const NV_ID *ctx)
{
        ...
        for(;;){
                ...
                if(!NV_NodeID_isEqual(&currentTermIndexNode,
&NODEID_NOT_FOUND)){
                        // do op
                        currentOpIndex =
NV_NodeID_getInt32(&currentTermIndexNode); currentTerm =
NV_Array_getByIndex(&currentBlock, currentOpIndex); if(IS_DEBUG_MODE()){
                                printf("op at currentBlock[%d]: ",
currentOpIndex); NV_Term_print(&currentTerm); putchar('\n');
                        }
                ...
                if(nextOpIndex == -1){
                        ...
                        // store last result
                        NV_NodeID_createUniqueIDRelation(ctx,
&RELID_LAST_RESULT, &t); continue;
                }
...
void NV_saveCodeGraphForVisualization(const NV_ID *codeGraphRoot,
